...
function Checkout() {
    ...
    async function oncheckout(e)
    {
        ...
        const checkoutdata = {saddr,tbill:sessionStorage.getItem("tbill"),uname:udata.username,pmode,carddetails,cartinfo};
        try
        {
    ...
    async function emptycart()
    {
        try
        {
            const resp =  await axios.delete("http://localhost:9000/api/deletecart?un=" + udata.username)
            if(resp.status===200)
            {
                if(resp.data.statuscode===0)
                {
